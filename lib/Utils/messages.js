...
import { downloadContentFromMessage, encryptedStream, generateThumbnail, getAudioDuration, getAudioWaveform, getRawMediaUploadData, prepareStream } from './messages-media.js';
const MIMETYPE_MAP = {
    image: 'image/jpeg',
    video: 'video/mp4',
...
export const prepareWAMessageMedia = async (message, options) => {
    ...
    const { mediaKey, encWriteStream, bodyPath, fileEncSha256, fileSha256, fileLength, didSaveToTmpPath, } = await (options.newsletter ? prepareStream : encryptedStream)(uploadData.media, options.mediaTypeOverride || mediaType, {
        logger,
    ...
    const [{ mediaUrl, directPath, handle }] = await Promise.all([
        ...
        (async () => {
            try {
                if (requiresThumbnailComputation) {
                    const { thumbnail, originalImageDimensions } = await (0, generateThumbnail)(bodyPath, mediaType, options);
                ...
                if (requiresDurationComputation) {
                    uploadData.seconds = await (0, getAudioDuration)(bodyPath);
                    logger === null || logger === void 0 ? void 0 : logger.debug('computed audio duration');
                }
                if (requiresWaveformProcessing) {
                    uploadData.waveform = await (0, getAudioWaveform)(bodyPath, logger);
        ...
        if (didSaveToTmpPath && bodyPath) {
            try {
                await promises.access(bodyPath);
    ...
    const obj = WAProto.Message.fromObject({
        [`${mediaType}Message`]: MessageTypeProto[mediaType].fromObject({
            ...
            fileSha256,
            fileLength,
            mediaKeyTimestamp: handle ? undefined : (0, unixTimestampSeconds)(),
            ...uploadData,
            media: undefined
        })
    });
...
export const generateWAMessageContent = async (message, options) => {
    ...
    if ('template' in message && !!message.templateButtons) {
